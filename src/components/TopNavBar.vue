  ...
  <header class="bg-surface dark:bg-surface-dim border-b border-outline-variant dark:border-outline shadow-sm dark:shadow-none sticky top-0 z-50">
    <div class="flex justify-between items-center w-full max-w-[1280px] mx-auto px-4 lg:px-margin-desktop h-20">
      ...
      <nav class="hidden lg:flex items-center gap-8">
        <RouterLink to="/admissions" class="text-on-surface-variant dark:text-on-surface-variant font-body-md hover:text-primary dark:hover:text-primary-fixed transition-colors duration-200 scale-100 active:scale-95 transition-transform">Admissions</RouterLink>
      ...
      <div class="hidden lg:flex items-center gap-4">
        ...
        <RouterLink to="/student-profile" class="text-on-surface hover:text-primary transition-colors p-2 flex items-center justify-center rounded-full hover:bg-surface-variant" title="Student Profile">
    ...
    <div v-if="isMobileMenuOpen" class="lg:hidden absolute top-20 left-0 w-full bg-surface border-b border-outline-variant shadow-lg py-4 px-4 flex flex-col gap-4 z-40 transition-all">
      <nav class="flex flex-col gap-4">
        <RouterLink @click="isMobileMenuOpen = false" to="/admissions" class="text-on-surface-variant font-body-lg font-medium hover:text-primary transition-colors py-2 border-b border-outline-variant/30">Admissions</RouterLink>
        ...
        <RouterLink @click="isMobileMenuOpen = false" to="/student-profile" class="text-on-surface-variant font-body-lg font-medium hover:text-primary transition-colors py-2 border-b border-outline-variant/30 flex items-center gap-2">
